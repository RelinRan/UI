...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent3, NestedScrollingParent2, NestedScrollingChild3, NestedScrollingChild2, NestedScrollingParent, NestedScrollingChild {
    ...
    public static final int LARGE = SwipeRefreshDrawable.LARGE;
    // Maps to ProgressBar default style
    public static final int DEFAULT = SwipeRefreshDrawable.DEFAULT;
    ...
    SwipeRefreshImageView mCircleView;
    ...
    SwipeRefreshDrawable mProgress;
    ...
    public void setSize(int size) {
        if (size != SwipeRefreshDrawable.LARGE && size != SwipeRefreshDrawable.DEFAULT) {
            return;
        }
        final DisplayMetrics metrics = getResources().getDisplayMetrics();
        if (size == SwipeRefreshDrawable.LARGE) {
    ...
    private void createProgressView() {
        mCircleView = new SwipeRefreshImageView(getContext());
        mProgress = new SwipeRefreshDrawable(getContext());
        mProgress.setStyle(SwipeRefreshDrawable.DEFAULT);
        mCircleView.setImageDrawable(mProgress);
        mCircleView.setVisibility(View.GONE);
        addView(mCircleView);
    }
